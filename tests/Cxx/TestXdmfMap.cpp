...
int main(int argc, char* argv[])
{
	...
	std::vector<boost::shared_ptr<XdmfMap> > boundaryMaps = XdmfMap::New(globalNodeIds);
	...
	boost::shared_ptr<XdmfDomain> domain2 = boost::shared_dynamic_cast<XdmfDomain>(reader->read("TestXdmfMap1.xmf"));
	...
	assert(XdmfTestCompareFiles::compareFiles("TestXdmfMap1.xmf", "TestXdmfMap2.xmf"));

	boost::shared_ptr<XdmfWriter> writerHDF = XdmfWriter::New("TestXdmfMapHDF1.xmf");
	writerHDF->setLightDataLimit(0);
	domain->accept(writerHDF);

	boost::shared_ptr<XdmfDomain> domainHDF = boost::shared_dynamic_cast<XdmfDomain>(reader->read("TestXdmfMapHDF1.xmf"));
	...
	boost::shared_ptr<XdmfWriter> writerHDF2 = XdmfWriter::New("TestXdmfMapHDF2.xmf");
	writerHDF2->getHDF5Writer()->setMode(XdmfHDF5Writer::Overwrite);
	writerHDF2->setLightDataLimit(0);
	domainHDF->accept(writerHDF2);

	assert(XdmfTestCompareFiles::compareFiles("TestXdmfMapHDF1.xmf", "TestXdmfMapHDF2.xmf"));
