...
#ifdef ARL_DEBUG_HDF5
static void* PrintCalloc(size_t s, size_t z)
{
  void* ptr = calloc(s, z);
  printf("0x%p - alloc (c)\n", ptr);
  fflush(stdout);
...
#define HDcalloc(N,Z)		PrintCalloc(N,Z)
#else /* ARL_DEBUG_HDF5 */
#define HDcalloc(N,Z)		calloc(N,Z)
#endif /* ARL_DEBUG_HDF5 */
...
#ifdef ARL_DEBUG_HDF5
#define HDfree(M)		{printf("0x%p - free\n", M);fflush(stdout);free(M);}
#else /* ARL_DEBUG_HDF5 */
...
#ifdef ARL_DEBUG_HDF5
static void* PrintMalloc(size_t s)
{
  void* ptr = malloc(s);
  printf("0x%p - alloc (m)\n", ptr);
  fflush(stdout);
...
#define HDmalloc(Z)		PrintMalloc(Z)
#else /* ARL_DEBUG_HDF5 */
...
#ifdef ARL_DEBUG_HDF5
static void* PrintRealloc(void* p, size_t s)
{
  void* ptr = realloc(p, s);
  printf("0x%p - free (realloc)\n", p);
  printf("0x%p - malloc (realloc)\n", ptr);
  fflush(stdout);
  return ptr;
}
#define HDrealloc(M,Z)		PrintRealloc(M,Z)
#else /* ARL_DEBUG_HDF5 */
...
#ifdef ARL_DEBUG_HDF5
static char* PrintStrdup(const char* str)
{
  char* ptr = strdup(str);
  printf("0x%p - alloc (s)\n", ptr);
  fflush(stdout);
...
#define HDstrdup(S)             PrintStrdup(S)
#else /* ARL_DEBUG_HDF5 */
