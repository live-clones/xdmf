...
int main()
{
  ...
  XDMFUNSTRUCTUREDGRID * unGrid = XdmfUnstructuredGridNewFromRegularGrid(regGrid, &status);
  XdmfRegularGridFree(regGrid);

  XDMFTIME * unTime = XdmfTimeNew(0.0);
  ...
  XDMFITEM * readItem = XdmfReaderRead(reader, "unfile.xmf", &status);
  XDMFUNSTRUCTUREDGRID * readGrid = XdmfUnstructuredGridCast(readItem);
  XdmfItemFree(readItem);
  XdmfReaderFree(reader);

  XDMFTIME * readTime = XdmfUnstructuredGridGetTime(readGrid);
  ...
  XdmfUnstructuredGridFree(readGrid);
