...
int main(int, char *)
{
  ...
  boost::shared_ptr<XdmfUnstructuredGrid> grid1 =
    XdmfTestDataGenerator::createHexahedron();
  boost::shared_ptr<XdmfUnstructuredGrid> grid2 =
    XdmfTestDataGenerator::createHexahedron();
  ...
  boost::shared_ptr<XdmfDomain> readDomain =
    ...
    (reader->read("TestXdmfReader1.xmf"));

  boost::shared_ptr<XdmfWriter> writer2 =
    XdmfWriter::New("TestXdmfReader2.xmf");
  ...
  std::vector<boost::shared_ptr<XdmfItem> > readItems =
    reader->read("TestXdmfReader1.xmf", "/Xdmf/Domain/Grid[1]");
  assert(readItems.size() == 1);
  boost::shared_ptr<XdmfUnstructuredGrid> readGrid =
    boost::shared_dynamic_cast<XdmfUnstructuredGrid>(readItems[0]);
  assert(readGrid->getName().compare("Hexahedron") == 0);

  std::vector<boost::shared_ptr<XdmfItem> > readItems2 =
    reader->read("TestXdmfReader1.xmf", "//Attribute");
  assert(readItems2.size() == 4);
  boost::shared_ptr<XdmfAttribute> readAttribute =
    boost::shared_dynamic_cast<XdmfAttribute>(readItems2[0]);
