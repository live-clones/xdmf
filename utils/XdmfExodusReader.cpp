...
boost::shared_ptr<XdmfUnstructuredGrid> XdmfExodusReader::read(const std::string & fileName, const boost::shared_ptr<XdmfHDF5Writer> heavyDataWriter) const
{
	boost::shared_ptr<XdmfUnstructuredGrid> toReturn = XdmfUnstructuredGrid::New();
...
boost::shared_ptr<const XdmfTopologyType> XdmfExodusReader::exodusToXdmfTopologyType(std::string exodusTopologyType, const int pointsPerCell) const
{
	...
	std::transform(exodusTopologyType.begin(), exodusTopologyType.end(), exodusTopologyType.begin(), toupper);

	// First check for quadratic elements then look for linear elements
	if (exodusTopologyType.substr(0,3).compare("TRI") == 0 && pointsPerCell == 6)
	{
		return XdmfTopologyType::Triangle_6();
	}
	else if (exodusTopologyType.substr(0,3).compare("SHE") == 0 && pointsPerCell == 8)
	{
		return XdmfTopologyType::Quadrilateral_8();
	}
	else if (exodusTopologyType.substr(0,3).compare("SHE") == 0 && pointsPerCell == 9)
	{
		return XdmfTopologyType::Quadrilateral_9();
	}
	else if (exodusTopologyType.substr(0,3).compare("TET") == 0 && pointsPerCell == 10)
	{
		return XdmfTopologyType::Tetrahedron_10();
	}
	else if (exodusTopologyType.substr(0,3).compare("TET") == 0 && pointsPerCell == 11)
	{
		// VTK_QUADRATIC_TETRA with 11 points
		// Currently unsupported in Xdmf
		return XdmfTopologyType::NoTopologyType();
	}
	else if (exodusTopologyType.substr(0,3).compare("WED") == 0 && pointsPerCell == 15)
	{
		return XdmfTopologyType::Wedge_15();
	}
	else if (exodusTopologyType.substr(0,3).compare("WED") == 0 && pointsPerCell == 18)
	{
		return XdmfTopologyType::Wedge_18();
	}
	else if (exodusTopologyType.substr(0,3).compare("HEX") == 0 && pointsPerCell == 20)
	{
		return XdmfTopologyType::Hexahedron_20();
	}
	else if (exodusTopologyType.substr(0,3).compare("HEX") == 0&& pointsPerCell == 21)
	{
		// VTK_QUADRATIC_HEXAHEDRON with 21 points
		// Currently unsupported in Xdmf
		return XdmfTopologyType::NoTopologyType();
	}
	else if (exodusTopologyType.substr(0,3).compare("HEX") == 0 && pointsPerCell == 27)
	{
		return XdmfTopologyType::Hexahedron_27();
	}
	else if (exodusTopologyType.substr(0,3).compare("QUA") == 0 && pointsPerCell == 8)
	{
		return XdmfTopologyType::Quadrilateral_8();
	}
	else if (exodusTopologyType.substr(0,3).compare("QUA") == 0 && pointsPerCell == 9)
	{
		return XdmfTopologyType::Quadrilateral_9();
	}
	else if (exodusTopologyType.substr(0,3).compare("TRU") == 0 && pointsPerCell == 3)
	{
		return XdmfTopologyType::Edge_3();
	}
	else if (exodusTopologyType.substr(0,3).compare("BEA") == 0 && pointsPerCell == 3)
	{
		return XdmfTopologyType::Edge_3();
	}
	else if (exodusTopologyType.substr(0,3).compare("BAR") == 0 && pointsPerCell == 3)
	{
		return XdmfTopologyType::Edge_3();
	}
	else if (exodusTopologyType.substr(0,3).compare("EDG") == 0 && pointsPerCell == 3)
	{
		return XdmfTopologyType::Edge_3();
	}
	...
	else if (exodusTopologyType.substr(0,3).compare("SHE") == 0 && pointsPerCell == 3)
	{
		return XdmfTopologyType::Triangle();
	}
	else if (exodusTopologyType.substr(0,3).compare("SHE") == 0 && pointsPerCell == 4)
	{
		return XdmfTopologyType::Quadrilateral();
	}
	else if (exodusTopologyType.substr(0,8).compare("STRAIGHT") == 0 && pointsPerCell == 2)
	{
		// VTK_LINE;
		// Currently unsupported in Xdmf
		return XdmfTopologyType::NoTopologyType();
