...
class XdmfSTLConverter {
	...
	public:
		...
		template <template <typename, typename> class Container, class T, class U>
		void inline getMapFromXdmf(Container<T, U> &myContainer, XdmfSet * currSet);

	private:
		template <class T>
		void inline copyFromXdmfArray(std::vector<T> &myContainer, XdmfArray * myIds);

		template <class T>
		void inline copyFromXdmfArray(std::deque<T> &myContainer, XdmfArray * myIds);

		template <class T>
		void inline copyFromXdmfArray(std::list<T> &myContainer, XdmfArray * myIds);

		template <class T>
		void inline copyFromXdmfArray(std::set<T> &myContainer, XdmfArray * myIds);

		template <class T>
		void inline copyFromXdmfArray(std::multiset<T> &myContainer, XdmfArray * myIds);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<char> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<short> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<int> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<long long> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<float> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<double> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<unsigned char> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<unsigned short> &myContainer);

		template <template <typename> class Container>
		void inline generateXdmfArray(XdmfArray * currentArray,  const Container<unsigned int> &myContainer);

		template <template <typename> class Container, class T>
		void inline writeArrayValues(XdmfArray * currentArray,  const Container<T> &myContainer);

		template <class T>
		T inline getArrayValue(XdmfArray * currentArray,  int index);
	};
	...
	void XdmfSTLConverter::getMapFromXdmf(Container<T, U> &myContainer, XdmfSet * currSet)
	{
		...
		{
			myContainer.insert(std::pair<T,U>(getArrayValue<T>(myKeys, i),getArrayValue<U>(myVals, i)));
		}
	}

	template <class T>
	void XdmfSTLConverter::copyFromXdmfArray(std::vector<T> &myContainer, XdmfArray * myIds)
	{
	...
	template <class T>
	void XdmfSTLConverter::copyFromXdmfArray(std::deque<T> &myContainer, XdmfArray * myIds)
	{
	...
	template <class T>
	void XdmfSTLConverter::copyFromXdmfArray(std::list<T> &myContainer, XdmfArray * myIds)
	{
		for (int j=0; j<myIds->GetNumberOfElements(); j++)
		{
			myContainer.push_back(getArrayValue<T>(myIds, j));
	...
	template <class T>
	void XdmfSTLConverter::copyFromXdmfArray(std::set<T> &myContainer, XdmfArray * myIds)
	{
		for (int j=0; j<myIds->GetNumberOfElements(); j++)
		{
	...
	template <class T>
	T XdmfSTLConverter::getArrayValue(XdmfArray * currentArray, int index)
	{
